...
std::string Minidump::format_arg_list(const char *fmt, va_list args) {
  if (!fmt) return "";
...
std::string Minidump::format_string(const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
std::wstring Minidump::s2ws(const std::string& s) {
  int len;
...
std::wstring Minidump::get_dump_filename() {
  time_t rawtime;
  struct tm timeinfo;
...
void Minidump::Start() {
  ...
  PreviousExceptionFilter = SetUnhandledExceptionFilter(my_top_level_filter);
}
